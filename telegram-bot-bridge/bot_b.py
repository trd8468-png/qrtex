...
def save_session(session_id, user_id, name="", username=""):
    with db() as conn:
        conn.execute("""INSERT INTO sessions(session_id,user_id,name,username,status,updated_at)
            VALUES(?,?,?,?, 'open', ?)
            ON CONFLICT(session_id) DO UPDATE SET
            user_id=excluded.user_id,
            name=CASE WHEN excluded.name='' THEN sessions.name ELSE excluded.name END,
            username=CASE WHEN excluded.username='' THEN sessions.username ELSE excluded.username END,
...
def get_session_for_admin_message(message_id):
    with db() as conn:
        row = conn.execute("SELECT session_id FROM replies WHERE admin_message_id=?",
                           (message_id,)).fetchone()
...
def get_open_sessions():
    with db() as conn:
        return conn.execute("""SELECT session_id,user_id,name,username
            FROM sessions WHERE status='open' ORDER BY updated_at DESC LIMIT 30""").fetchall()

async def send_to_bot_a(context, text):
    return await context.bot.send_message(chat_id=f"@{BOT_A_USERNAME}", text=text)

def close_markup(session_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔴 Close conversation", callback_data=f"close:{session_id}")]
    ])
...
async def bot_a_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not message or not message.from_user or not message.from_user.is_bot:
        return
    if (message.from_user.username or "").lower() != BOT_A_USERNAME.lower():
        return
    ...
    if command == "BRIDGE_REQUEST":
        ...
        try:
            await send_to_bot_a(
                context,
                f"BRIDGE_ACK\nSESSION:{session_id}\n"
                "MESSAGE:✅ Your support request has reached our team. Please wait for a manual reply."
            )
        except Exception:
...
async def admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not session_id:
        await message.reply_text("❌ That message is not linked to an active support session.")
        return
    ...
    try:
        ...
        await message.reply_text("❌ Reply failed. Check Bot-to-Bot Communication Mode on both bots.")
...
async def close_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await send_to_bot_a(context, f"BRIDGE_CLOSE\n{session_id}")
        close_session_local(session_id)
        if query.message:
            await query.message.edit_reply_markup(reply_markup=None)
            await query.message.reply_text(f"🔴 Conversation {session_id} closed.")
    except Exception:
        logger.exception("Close failed.")
        if query.message:
            await query.message.reply_text("❌ Could not close this conversation.")
...
async def sessions_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for row in rows:
        lines.append(
            f"• {row['session_id']}\n"
            f"  {row['name'] or 'Unknown'} | {row['username'] or 'no username'}\n"
...
def main():
    ...
    app.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, admin_reply), group=1)
    logger.info("BOT B is running. Bridge source: @%s", BOT_A_USERNAME)
